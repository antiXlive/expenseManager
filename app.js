/* ===========================================================================
   ...
   - No behavioral changes (only safety & small optimizations)
   =========================================================================== */
...
if (state.settings.bio && localStorage.getItem(PASSKEY_ID)) {
    biometricUnlock().then(success => {
        if (success) $("#lock").classList.add("hidden");
    });
}
...
/* ===========================================================================
   SECTION 9 — BIOMETRICS & LOCK SCREEN
   =========================================================================== */
...
async function registerBiometricKey() {
    try {
        const publicKey = {
            challenge: randomBuf(),
            rp: { name: "Expense Manager", id: location.hostname || "localhost" },
            user: {
        ...
        localStorage.setItem(PASSKEY_ID, id);
...
async function biometricUnlock() {
    const storedId = localStorage.getItem(PASSKEY_ID);
...
/* lock screen setup — safe with guards */
function setupLock() {
    const ls = $("#lock");
    ...
    if (alt) {
        alt.onclick = async () => {
            ...
            if (success) {
                ...
                alert("Biometric failed. Try again.");
            }
        };
    }
...
/* ===========================================================================
   SECTION 11 — BOOTSTRAP: Wiring event listeners + init
   =========================================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#btn-toggle-bio").onclick = async () => {
        if (!localStorage.getItem(PASSKEY_ID)) {
            const ok = await registerBiometricKey();
            if (!ok) return alert("Failed to create biometric credential.");
        }
